...
| Set | Marking |
|-----|---------|
| `flat` (default) | coloured animals drawn for this game |
| `body` | 🐀 🐈 🐕 🐺 🐆 🐅 🦁 🐘 |
| `zh` | 鼠 猫 狗 狼 豹 虎 狮 象 |

The coloured set is why the emoji one can stay imperfect: eight animals in one
style, generated by `tools/pieces.py` into `<symbol>`s in the page and used via
`<use href="#pc_f6">`. It is the only complete set, and unlike emoji it looks
identical on every device rather than being redrawn by Apple, Google and
Microsoft. Edit an animal in the generator and run `python3 tools/pieces.py`
from the project root; it rewrites the sprite in `index.html` in place.

The generator can still emit a glossy, gradient-shaded finish - it was dropped
from the game to keep the choice short, not because it stopped working. Pass
`install(finishes=("flat", "glossy"))` to bring it back, and re-add the option
to `STYLES` and `DRAWN` in `app.js`.
...
- Paint is set with **presentation attributes on the shapes**, not CSS classes.
  ...
  leave every piece in the fallback colour.
- The sprite must stay **in the render tree** (`width="0" height="0"`, not
  `display:none`). A `display:none` subtree does not resolve as a paint server,
  so every gradient-filled head silently renders as nothing at all.
- Each drawn piece carries its own cream medallion, so the red or black token
  shows only as a **ring** around it. That ring is the one thing marking whose
  piece it is, so it is deliberately thick and strongly coloured rather than
  the thin pale border the emoji sets can get away with.

**A complete emoji set is impossible.** Scanning the whole emoji block turns up
no lion body and no wolf body - `🦁 U+1F981 LION FACE` and `🐺 U+1F43A WOLF
FACE` are the only pictures that exist for those two. So in `body` they show as
heads (they are listed in `STUCK`), and the picker puts a dot under them rather
than quietly shipping a set that looks broken.

`body` also cannot separate the tiger from the leopard - they are
near-identical striped/spotted quadrupeds at board size. That is what the drawn
set exists to fix.
