...
Pieces can be drawn five ways, chosen in the menu or cycled mid-game from the
board controls:

| Set | Marking |
|-----|---------|
| `glossy` (default) | drawn for this game, shaded like a moulded toy |
| `flat` | the same drawings in flat cartoon colour |
| `face` | 🐭 🐱 🐶 🐺 🐆 🐯 🦁 🐘 |
| `body` | 🐀 🐈 🐕 🐺 🐆 🐅 🦁 🐘 |
| `zh` | 鼠 猫 狗 狼 豹 虎 狮 象 |

The two coloured sets are why the emoji ones can stay imperfect: eight animals
in one style, generated by `tools/pieces.py` into `<symbol>`s in the page and
used via `<use href="#pc_g6">`. They are the only complete sets, and unlike
emoji they look identical on every device rather than being redrawn by Apple,
Google and Microsoft. Edit an animal in the generator and re-run it; it
rewrites the sprite in `index.html` in place.
...
they are near-identical spotted/striped quadrupeds at board size. Prefer the
drawn sets, or `zh`, which is what a real board uses.
